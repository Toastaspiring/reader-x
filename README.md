...
- **Table of contents.** Jump straight to any chapter (EPUB navigation or PDF outline).
- **Three themes.** Light, Sepia and Dark, applied to the app and to the book's pages.
- **Search and sort.** Filter by title or author; sort by Recent, Title or Date added.
- **Real EPUB rendering.** Inline images, chapter styling and embedded fonts, powered by epub.js.
...
> Requires Android 7.0 (API 24) or newer. The APK is self-signed, so the installer warns
> that it comes from an unknown developer. That is expected for a sideloaded app.

---

## Run the web app

ReaderX is a plain static site with no build step and nothing to install.
...
| Mode | Gesture |
|---|---|
| **Pages** | Tap the left or right screen edge, or swipe, to turn the page |
...
| Key | Action |
|---|---|
| `Right` `Left` `Space` | Next or previous page |
| `+` `-` | Text size or zoom |
...
| `js/storage.js` | IndexedDB wrapper for books and reading progress |
| `js/library.js` | Import, metadata, cover thumbnails, the grid |
| `js/epub-reader.js` | EPUB rendering and pagination (epub.js) |
...
Built on [epub.js](https://github.com/futurepress/epub.js), [pdf.js](https://mozilla.github.io/pdf.js/)
and [JSZip](https://stuk.github.io/jszip/). No framework, no bundler.
...
The release build is signed with a local keystore (`keystore.properties` plus a `signingConfig`
...
Key Android settings: `minSdk 24` and `targetSdk 35`, so the edge-to-edge opt-out is honoured
...
<div align="center">
<sub>ReaderX. Built with epub.js and pdf.js.</sub>
</div>
